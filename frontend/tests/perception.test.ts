...
describe("perception", () => {
  it("keeps following robots >= PERC_STOP apart and reports obstacles", () => {
    ...
    for (let t = 0; t < 6000; t++) {
      ...
      for (let i = 0; i < rs.length; i++) for (let j = i + 1; j < rs.length; j++) { const a = rs[i], b = rs[j]; const d = Math.hypot(a.position[0] - b.position[0], a.position[2] - b.position[2]); if (d < minD) minD = d; }
    }
